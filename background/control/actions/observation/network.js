...
export class NetworkActions extends BaseActionHandler {
    ...
    async listNetworkRequests({ resourceTypes, limit = 20 }) {
        ...
        return requests
            .map(
                (r) => `ID: ${r.id} | ${r.method} ${r.url} | Status: ${r.status} | Type: ${r.type}`
            )
            .join('\n');
    }
    ...
    async getNetworkRequest({ requestId }) {
        ...
        if (!req)
            return `Error: Request ID ${requestId} not found. Use list_network_requests first.`;
                ...
                if (res.base64Encoded) {
                    ...
                    if (
                        ...
                        (req.mimeType.includes('json') ||
                            req.mimeType.includes('text') ||
                            req.mimeType.includes('xml'))
                    ) {
                        try {
                            body = atob(res.body);
                        } catch (e) {
