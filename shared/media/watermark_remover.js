...
// Singleton to hold loaded alpha maps
let instance = null;
...
class WatermarkRemoverEngine {
    ...
    static async create() {
        ...
        const [bg48, bg96] = await Promise.all([loadImage(bg_48_b64), loadImage(bg_96_b64)]);

        instance = new WatermarkRemoverEngine(bg48, bg96);
        return instance;
    }

    calculateAlphaMap(bgImage, width, height) {
        ...
        for (let i = 0; i < alphaMap.length; i++) {
            const idx = i * 4;
            // Max channel approach from userscript
            const r = data[idx];
            const g = data[idx + 1];
            const b = data[idx + 2];
    ...
    getAlphaMap(size) {
        ...
        const map = this.calculateAlphaMap(bgImage, size, size);
    ...
    async remove(base64Image) {
        ...
        const alphaMap = this.getAlphaMap(logoSize);
        ...
        const startX = width - marginRight - logoSize;
        const startY = height - marginBottom - logoSize;
        ...
        for (let row = 0; row < logoSize; row++) {
            for (let col = 0; col < logoSize; col++) {
                ...
                for (let c = 0; c < 3; c++) {
                    ...
                    // Reverse blending: Original = (Watermarked - Alpha * LogoColor) / (1 - Alpha)
...
export class WatermarkRemover {
    static async process(base64Image) {
        const engine = await WatermarkRemoverEngine.create();
        return engine.remove(base64Image);
    }
}
